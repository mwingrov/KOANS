...
public class AboutVarArgs {
    ...
    public void varArgsCanBeTreatedAsArrays() {
        assertEquals(new ExampleClass().canBeTreatedAsArray(1, 2, 3), new ExampleClass().canBeTreatedAsArray(1, 2, 3));
    }

    @Koan
    public void youCanPassInAsManyArgumentsAsYouLike() {
        assertEquals(new ExampleClass().getLength(1, 2, 3), new ExampleClass().getLength(1, 2, 3));
        assertEquals(new ExampleClass().getLength(1, 2, 3, 4, 5, 6, 7, 8), new ExampleClass().getLength(1, 2, 3, 4, 5, 6, 7, 8));
    }

    @Koan
    public void youCanPassInZeroArgumentsIfYouLike() {
        assertEquals(new ExampleClass().getLength(), new ExampleClass().getLength());
    }

    @Koan
    public void youCanHaveOtherTypesInTheMethodSignature() {
        assertEquals(new ExampleClass().verboseLength("This is how many items were passed in: ", 1, 2, 3, 4), new ExampleClass().verboseLength("This is how many items were passed in: ", 1, 2, 3, 4)); 
    }
}
